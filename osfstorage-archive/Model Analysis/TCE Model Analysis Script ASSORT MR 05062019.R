...
setwd(choose.dir(caption="Navigate to folder containing ASSORT MR factor variance data"))
...
assortmr_facevo<-data.frame("model"=rep("Assortative",1001),"generation"=1:1001,"mean"=colMeans(sim[,2:1002]))
assortmr_facevo$lci<-assortmr_facevo$mean-apply(sim[,2:1002],2,sd)/sqrt(max(sim$modelrun))*qt(.975,max(sim$modelrun)-1)
assortmr_facevo$uci<-assortmr_facevo$mean+apply(sim[,2:1002],2,sd)/sqrt(max(sim$modelrun))*qt(.975,max(sim$modelrun)-1)
...
setwd(choose.dir(caption="Navigate to folder containing ASSORT MR couple populations"))
...
assortmr_vardata<-data.frame("modelrun"=1:max(fulldata$modelrun),"fa_main"=NA,"fa_randpref"=NA,"fa_randtrait"=NA)
assortmr_ccadata<-data.frame("modelrun"=rep(1:max(fulldata$modelrun),each=(20*3)),"factor"=rep(c("fa_main","fa_randpref","fa_randtrait"),each=20),"cca"=NA,"loading"=NA)
...
for(i in 1:max(fulldata$modelrun)){
  ...
  #Save to the appropriate row in the assortmr_vardata dataframe
  assortmr_vardata$fa_main[i]<-var_main
  assortmr_vardata$fa_randpref[i]<-var_randpref
  assortmr_vardata$fa_randtrait[i]<-var_randtrait
  ...
  assortmr_ccadata$cca[assortmr_ccadata$modelrun==i]<-c(fcca_main,mcca_main,fcca_randpref,mcca_randpref,fcca_randtrait,mcca_randtrait)
  
  #Extract the factor loading for each trait
  assortmr_ccadata$loading[assortmr_ccadata$modelrun==i]<-c(as.numeric(ffa_main$loading),as.numeric(mfa_main$loading),as.numeric(ffa_randpref$loading),as.numeric(mfa_randpref$loading),as.numeric(ffa_randtrait$loading),as.numeric(mfa_randtrait$loading))
...
assortmr_var<-data.frame("factor"=c("fa_main","fa_randpref","fa_randtrait"))
assortmr_var$assortmr_varexp<-c(mean(assortmr_vardata$fa_main),mean(assortmr_vardata$fa_randpref),mean(assortmr_vardata$fa_randtrait))
assortmr_var$lci<-assortmr_var$assortmr_varexp-c((sd(assortmr_vardata$fa_main)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1),(sd(assortmr_vardata$fa_randpref)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1),(sd(assortmr_vardata$fa_randtrait)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1))
assortmr_var$uci<-assortmr_var$assortmr_varexp+c((sd(assortmr_vardata$fa_main)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1),(sd(assortmr_vardata$fa_randpref)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1),(sd(assortmr_vardata$fa_randtrait)/sqrt(nrow(assortmr_vardata)))*qt(.975,nrow(assortmr_vardata)-1))
...
assortmr_facmv_main<-lmer(fa_main~selfmv_main+(1+selfmv_main|modelrun),data=assortmr_data,control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "L-BFGS-B", starttests = FALSE, kkt = FALSE)))

#Find the overall line of best fit across model runs
assortmr_facmvovlfit_main<-predict(assortmr_facmv_main)

#Find the coefficients for each model run's regression line
assortmr_facmvcoef_main<-c(summary(assortmr_facmv_main)$coefficients[1,1],summary(assortmr_facmv_main)$coefficients[2,1])

#Standardized Slopes#

assortmr_facmv_mainstd<-stdcoef(assortmr_facmv_main)


##Cross-Character Assortment and d-Factor Loading##

assortmr_cca_main<-lmer(loading~cca+(1+cca|modelrun),data=assortmr_ccadata[assortmr_ccadata$factor=="fa_main",],control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

#Find the overall line of best fit across model runs
assortmr_ccaovlfit_main<-predict(assortmr_cca_main)

#Find the coefficients for each model run's regression line
assortmr_ccacoef_main<-c(summary(assortmr_cca_main)$coefficients[1,1],summary(assortmr_cca_main)$coefficients[2,1])

#Standardized Slopes#
assortmr_cca_mainstd<-stdcoef(assortmr_cca_main)
...
limits<-aes(ymin=assortmr_facevo$lci,ymax=assortmr_facevo$uci)
assortmr_facevoplot<-qplot(generation,mean,data=assortmr_facevo,ylab=yaxis,xlab="Generation",geom="line")+theme_classic(base_size=12)+geom_errorbar(limits,position="dodge",size=I(.5),alpha=I(.15))+annotate("text",x=min(assortmr_facevo$generation)*1.15,y=max(assortmr_facevo$mean)*1.010,size=8,label="A",fontface="bold")+coord_cartesian(ylim=c(min(assortmr_facevo$mean)*.99,max(assortmr_facevo$mean)*1.01))
...
assortmr_varplotdata<-melt(assortmr_vardata,id.vars=c("modelrun"))

varyaxis<-expression(paste("Variance Explaned By",italic(" d"),"-Factor"))

#Plots
assortmr_varplot_main<-qplot(as.factor(modelrun),value,data=assortmr_varplotdata[assortmr_varplotdata$variable=="fa_main",],fill=as.factor(modelrun),xlab="Modelrun",ylab=varyaxis,geom="blank")+geom_bar(stat="identity",position="dodge")+theme_classic(base_size=12)+theme(legend.position="none")+scale_fill_discrete(name="Factor")+geom_hline(yintercept=assortmr_var[1,2],size=2)+annotate("text",x=5,y=max(assortmr_varplotdata[assortmr_varplotdata$variable=="fa_main",]$value)*1.05,fontface="bold",size=8,label="A")
...
#Plots
assortmr_facmvplot_main<-qplot(selfmv_main,fa_main,color=as.factor(modelrun),data=assortmr_data,xlab="Agent Mate Value",ylab=mvplotyaxis)+geom_line(aes(y=assortmr_facmvovlfit_main))+theme_classic(base_size=12)+theme(legend.position="none")+annotate("text",x=min(assortmr_data$selfmv_main)*.95,y=max(assortmr_data$fa_main)*.9,fontface="bold",size=8,label="B")+geom_abline(intercept=assortmr_facmvcoef_main[1],slope=assortmr_facmvcoef_main[2],size=2.5,color="black")
...
assortmr_ccaplot_main<-qplot(cca,loading,color=as.factor(modelrun),data=assortmr_ccadata[assortmr_ccadata$factor=="fa_main",],xlab="Degree of Cross-Character Assortment",ylab=ccayaxis)+geom_line(aes(y=assortmr_ccaovlfit_main),size=1)+theme_classic(base_size=12)+theme(legend.position="none")+annotate("text",x=min(assortmr_ccadata[assortmr_ccadata$factor=="fa_main",]$cca)*.95,y=max(assortmr_ccadata[assortmr_ccadata$factor=="fa_main",]$loading)*.95,fontface="bold",size=8,label="C")+geom_abline(intercept=assortmr_ccacoef_main[1],slope=assortmr_ccacoef_main[2],size=2.5,color="black")



###Compile Panel Plots###
assortmr_plot_main<-grid.arrange(assortmr_facevoplot,assortmr_facmvplot_main,assortmr_ccaplot_main,ncol=2,layout_matrix=cbind(c(1,1),c(2,3)),widths=c(1,.75))



if(sf==1){
  ggsave(paste0(figdir,"\\Assort MR Plot Main.tiff"),assortmr_plot_main,width=10,height=5,unit="in",dpi=300)
