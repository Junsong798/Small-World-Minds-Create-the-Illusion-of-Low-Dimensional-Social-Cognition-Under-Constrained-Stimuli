...
setwd(choose.dir(caption="Navigate to folder containing RAND factor variance data"))
...
rand_facevo<-data.frame("model"=rep("Assortative",1001),"generation"=1:1001,"mean"=colMeans(sim[,2:1002]))
rand_facevo$lci<-rand_facevo$mean-apply(sim[,2:1002],2,sd)/sqrt(max(sim$modelrun))*qt(.975,max(sim$modelrun)-1)
rand_facevo$uci<-rand_facevo$mean+apply(sim[,2:1002],2,sd)/sqrt(max(sim$modelrun))*qt(.975,max(sim$modelrun)-1)
...
setwd(choose.dir(caption="Navigate to folder containing RAND couple populations"))
...
rand_vardata<-data.frame("modelrun"=1:max(fulldata$modelrun),"fa_main"=NA,"fa_randpref"=NA,"fa_randtrait"=NA)
rand_ccadata<-data.frame("modelrun"=rep(1:max(fulldata$modelrun),each=(20*3)),"factor"=rep(c("fa_main","fa_randpref","fa_randtrait"),each=20),"cca"=NA,"loading"=NA)
...
for(i in 1:max(fulldata$modelrun)){
  ...
  #Save to the appropriate row in the rand_vardata dataframe
  rand_vardata$fa_main[i]<-var_main
  rand_vardata$fa_randpref[i]<-var_randpref
  rand_vardata$fa_randtrait[i]<-var_randtrait
  ...
  rand_ccadata$cca[rand_ccadata$modelrun==i]<-c(fcca_main,mcca_main,fcca_randpref,mcca_randpref,fcca_randtrait,mcca_randtrait)
  
  #Extract the factor loading for each trait
  rand_ccadata$loading[rand_ccadata$modelrun==i]<-c(as.numeric(ffa_main$loading),as.numeric(mfa_main$loading),as.numeric(ffa_randpref$loading),as.numeric(mfa_randpref$loading),as.numeric(ffa_randtrait$loading),as.numeric(mfa_randtrait$loading))
...
rand_var<-data.frame("factor"=c("fa_main","fa_randpref","fa_randtrait"))
rand_var$rand_varexp<-c(mean(rand_vardata$fa_main),mean(rand_vardata$fa_randpref),mean(rand_vardata$fa_randtrait))
rand_var$lci<-rand_var$rand_varexp-c((sd(rand_vardata$fa_main)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1),(sd(rand_vardata$fa_randpref)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1),(sd(rand_vardata$fa_randtrait)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1))
rand_var$uci<-rand_var$rand_varexp+c((sd(rand_vardata$fa_main)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1),(sd(rand_vardata$fa_randpref)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1),(sd(rand_vardata$fa_randtrait)/sqrt(nrow(rand_vardata)))*qt(.975,nrow(rand_vardata)-1))
...
rand_facmv_main<-lmer(fa_main~selfmv_main+(1+selfmv_main|modelrun),data=rand_data,control = lmerControl(optimizer = "optimx", calc.derivs = FALSE,optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

#Find the overall line of best fit across model runs
rand_facmvovlfit_main<-predict(rand_facmv_main)

#Find the coefficients for each model run's regression line
rand_facmvcoef_main<-c(summary(rand_facmv_main)$coefficients[1,1],summary(rand_facmv_main)$coefficients[2,1])

#Standardized Slopes#

rand_facmv_mainstd<-stdcoef(rand_facmv_main)


##Cross-Character Assortment and d-Factor Loading##

rand_cca_main<-lmer(loading~cca+(1+cca|modelrun),data=rand_ccadata[rand_ccadata$factor=="fa_main",])

#Find the overall line of best fit across model runs
rand_ccaovlfit_main<-predict(rand_cca_main)

#Find the coefficients for each model run's regression line
rand_ccacoef_main<-c(summary(rand_cca_main)$coefficients[1,1],summary(rand_cca_main)$coefficients[2,1])

#Standardized Slopes#

rand_cca_mainstd<-stdcoef(rand_cca_main)
...
limits<-aes(ymin=rand_facevo$lci,ymax=rand_facevo$uci)
rand_facevoplot<-qplot(generation,mean,data=rand_facevo,ylab=yaxis,xlab="Generation",geom="line")+theme_classic(base_size=12)+geom_errorbar(limits,position="dodge",size=I(.5),alpha=I(.15))+annotate("text",x=min(rand_facevo$generation)*1.15,y=max(rand_facevo$mean)*1.010,size=8,label="A",fontface="bold")+coord_cartesian(ylim=c(min(rand_facevo$mean)*.99,max(rand_facevo$mean)*1.01))
...
rand_varplotdata<-melt(rand_vardata,id.vars=c("modelrun"))

varyaxis<-expression(paste("Variance Explaned By",italic(" d"),"-Factor"))

#Plots
rand_varplot_main<-qplot(as.factor(modelrun),value,data=rand_varplotdata[rand_varplotdata$variable=="fa_main",],fill=as.factor(modelrun),xlab="Modelrun",ylab=varyaxis,geom="blank")+geom_bar(stat="identity",position="dodge")+theme_classic(base_size=12)+theme(legend.position="none")+scale_fill_discrete(name="Factor")+geom_hline(yintercept=rand_var[1,2],size=2)+annotate("text",x=5,y=max(rand_varplotdata[rand_varplotdata$variable=="fa_main",]$value)*1.05,fontface="bold",size=8,label="A")
...
#Plots
rand_facmvplot_main<-qplot(selfmv_main,fa_main,color=as.factor(modelrun),data=rand_data,xlab="Agent Mate Value",ylab=mvplotyaxis)+geom_line(aes(y=rand_facmvovlfit_main))+theme_classic(base_size=12)+theme(legend.position="none")+annotate("text",x=min(rand_data$selfmv_main)*.95,y=max(rand_data$fa_main)*.9,fontface="bold",size=8,label="B")+geom_abline(intercept=rand_facmvcoef_main[1],slope=rand_facmvcoef_main[2],size=2.5,color="black")
...
rand_ccaplot_main<-qplot(cca,loading,color=as.factor(modelrun),data=rand_ccadata[rand_ccadata$factor=="fa_main",],xlab="Degree of Cross-Character Assortment",ylab=ccayaxis)+geom_line(aes(y=rand_ccaovlfit_main),size=1)+theme_classic(base_size=12)+theme(legend.position="none")+annotate("text",x=min(rand_ccadata[rand_ccadata$factor=="fa_main",]$cca)*.95,y=max(rand_ccadata[rand_ccadata$factor=="fa_main",]$loading)*.95,fontface="bold",size=8,label="C")+geom_abline(intercept=rand_ccacoef_main[1],slope=rand_ccacoef_main[2],size=2.5,color="black")



###Compile Panel Plots###
rand_plot_main<-grid.arrange(rand_facevoplot,rand_facmvplot_main,rand_ccaplot_main,ncol=2,layout_matrix=cbind(c(1,1),c(2,3)),widths=c(1,.75))



if(sf==1){
  ggsave(paste0(figdir,"\\Rand Plot Main.tiff"),rand_plot_main,width=10,height=5,unit="in",dpi=300)
}
