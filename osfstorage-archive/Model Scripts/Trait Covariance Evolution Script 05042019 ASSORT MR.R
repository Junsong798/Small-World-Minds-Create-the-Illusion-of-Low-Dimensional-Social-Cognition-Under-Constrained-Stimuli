...
fvpath<-paste0(fvpath,"\\TCE ASSORT MR Factor Variance ")
...
cppath<-paste0(cppath,"\\TCE ASSORT MR Couple Population ")
...
genegenerate<-function(popsize){
  if(popsize>0){
    ...
    # 这里生成trait的基因，均值0.04，每个trait由100个基因求和得到
    # 这里的1000是代数? 不是，应该是10*100个基因
...
agentgenerate<-function(agentgenes,sex,fitvec){
  ...
  # 调整的trait均值是4，最优值在1-7之间，这导致有些trait的最优值在中心
  # 这样就归一化了选择压力，不管optimal的位置在哪里
...
attraction<-function(agentprefs,targettraits){
  ...
  # 同理，把att映射到0-10的空间, 先映射到0-1，然后乘以10
  # 这里的6的平方是可能的最大距离，乘以10个trait
