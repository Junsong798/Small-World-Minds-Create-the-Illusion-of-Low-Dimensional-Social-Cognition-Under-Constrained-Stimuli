...
# 
# Reproduction
# Paired couples will reproduce. In this model, couples will produce offspring independent of their "energy" values and therefore randomly.
# Offspring will inherit trait and preference genes from their parents. For each trait and preference, offspring will randomly inherit the genes of either their mother or father.
...
fvpath<-paste0(fvpath,"\\TCE NOSEL Factor Variance ")
...
cppath<-paste0(cppath,"\\TCE NOSEL Couple Population ")
...
agentgenerate<-function(agentgenes,sex){
...
reproduceoffspring<-function(offgenes){
...
foreach(m=1:modelloops,.packages=c("plyr","psych")) %dopar% {
  ...
  #Generate agents
  males<-agentgenerate(malegenes,1)
  females<-agentgenerate(femalegenes,0)
  ...
  for(g in 1:generations){
    ...
    repro<-pairs[sample(nrow(pairs),popsize,replace=T),]
    ...
    offspring<-reproduceoffspring(offgenes)
