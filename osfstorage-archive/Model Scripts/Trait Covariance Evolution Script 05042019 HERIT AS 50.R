...
#Heritability#
#Sets the heritability of preferences and traits
#This value will be the proportion of variance in traits and preferences that can be explained by inherited values
h<-.50
...
fvpath<-paste0(fvpath,"\\TCE HERIT AS 50 Factor Variance ")
...
cppath<-paste0(cppath,"\\TCE HERIT AS 50 Couple Population ")
...
agentgenerate<-function(agentgenes,sex,fitvec,h){
  ...
  #Generate manifest trait values by adding random noise such that inherited trait values explain h% of the variance in manifest trait values
  mtraits<-apply(traits,2,function(x) x+rnorm(length(x),0,sqrt(sd(x)^2/h-sd(x)^2)))
  colnames(mtraits)<-paste(paste('mtrait',1:ncol(traits),sep=""))
  ...
  #Generate manifest preference values by adding random noise such that inherited preference values explain h% of the variance in manifest preference values
  mpreferences<-apply(preferences,2,function(x) x+rnorm(length(x),0,sqrt(sd(x)^2/h-sd(x)^2)))
  colnames(mpreferences)<-paste(paste('mpreference',1:ncol(preferences),sep=""))
  ...
  energy<-apply(mtraits,1,function(x) sum(10-(abs(fitvec-x)*fitvecweights)))
  
  sex<-sex
  
  return(as.data.frame(cbind(mtraits,mpreferences,energy,sex)))
}
...
reproduceoffspring<-function(offgenes,fitvec,h){
  ...
  offspring[,1:20]<-cbind(mtraits,mpreferences)
...
foreach(m=1:modelloops,.packages=c("plyr","psych")) %dopar% {
  ...
  #Generate agents
  males<-agentgenerate(malegenes,1,fitvec,h)
  females<-agentgenerate(femalegenes,0,fitvec,h)
  ...
  for(g in 1:generations){
    ...
    for(p in 1:(popsize/2)){
      ...
      maxmut<-which(mutattmatrix==max(mutattmatrix,na.rm=T),arr.ind=T)
    ...
    offspring<-reproduceoffspring(offgenes,fitvec,h)
